...
export default function AdminDashboard() {
  const {
    tips, loading, stats, fetchTips, exportCSV, dbAuthenticated,
    page, totalPages, setPage, setDateRange, filters, setSearch
  } = useAdminData();

  const [activeTab, setActiveTab] = useState<'tips' | 'waiters'>('tips');
  ...
  const handleExport = () => {
    ...
    // Por ahora mantenemos comportamiento: exporta lo visible.
    exportCSV(tips);
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex space-x-1 border-b border-monalisa-gold/20 pb-1">
          <button
            ...
            className={`px-6 py-2 text-sm font-medium transition-all rounded-t-sm relative ${activeTab === 'tips'
          ...
          <button
            ...
            className={`px-6 py-2 text-sm font-medium transition-all rounded-t-sm relative ${activeTab === 'waiters'
                ? 'text-monalisa-gold after:absolute after:bottom-[-5px] after:left-0 after:w-full after:h-[2px] after:bg-monalisa-gold'
                : 'text-monalisa-silver/50 hover:text-monalisa-silver hover:bg-white/5'
              }`}
          >
